// MetaPay - complete-task edge function
...
serve(async (req) => {
  ...
  try {
    ...
    if ((todayCount ?? 0) >= task.daily_limit) {
      return new Response(JSON.stringify({ error: 'Daily task limit reached for this task' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await supabase.from('transactions').insert({
      ...
      balance_before: profile.withdrawal_balance, balance_after: newBalance,
      ...
      if (commission > 0) {
        ...
        if (referral) {
          ...
          if (referrer) {
            ...
            await supabase.from('earnings').insert({
              user_id: profile.referred_by, amount: commission, source: 'referral', source_id: user.id,
